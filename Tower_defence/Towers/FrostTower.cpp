...
void FrostTower::fireBullet(const sf::Vector2f& targetPosition, float bulletSpeed) {
    if (canAttack()) {
        float slowFactor = BASIC_SLOW_FACTOR + SLOW_FACTOR_PER_LEVEL * currentLevel;
        float areaEffectRadius = 50.f;
        bullets.emplace_back(std::make_unique<FrostBullet>(shape.getPosition(), targetPosition, bulletSpeed, damage, slowFactor, areaEffectRadius, 2.0f));
        resetCooldown();
    }
}
